                ...
                if depth == 3 {
                    match (*a, *b) {
                        (Number::Pair(a, b), mut c) => {
                            *exploded = true;
                            let a = if let Number::Literal(v) = *a {
                                v
                            } else {
                                unreachable!()
                            };
                            let b = if let Number::Literal(v) = *b {
                                v
                            } else {
                                unreachable!()
                            };
                        ...
                        (mut a, Number::Pair(b, c)) => {
                            *exploded = true;
                            ...
                            let c = if let Number::Literal(v) = *c {
                                v
                            } else {
                                unreachable!()
                            };
...
fn main() {
    ...
    for i in 0..numbers.len() {
        for j in i + 1..numbers.len() {
            let first = Number::sum(numbers[i].clone(), numbers[j].clone())
                .reduce()
                .magnitude();

            let second = Number::sum(numbers[j].clone(), numbers[i].clone())
                .reduce()
                .magnitude();

            maximum_magnitude = maximum_magnitude.max(first.max(second));
        }
    }

    println!("{}", maximum_magnitude);
}

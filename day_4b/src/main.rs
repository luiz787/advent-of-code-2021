...
struct Bingo {
    ...
    played_rounds: u8,
    boards: Vec<Board>,
    count_of_remaining_boards: u8
}

impl Bingo {
    fn play_round(&mut self, round: u8) -> Option<u64> {
        let current_number = self.drawn_numbers.get(self.played_rounds as usize).unwrap();

        for board in &mut self.boards {
            if board.winning_round.is_none()  {
                let won = board.play_round(*current_number, round);
                if won {
                    self.count_of_remaining_boards -= 1;

                    if self.count_of_remaining_boards == 0 {
...
impl Board {
    fn play_round(&mut self, number: u8, round: u8) -> bool {
...
fn main() {
    ...
    let winning_board_score = loop {
        let round_result = bingo.play_round(round);
...
fn parse_input(input: &str) -> Bingo {
    ...
    for _i in 0..board_count {
        ...
        boards.push(Board { values: board_cells, winning_number: None, winning_round: None });
    }

    let count_of_remaining_boards = boards.len() as u8;

    Bingo { boards, drawn_numbers, played_rounds: 0, count_of_remaining_boards }
}

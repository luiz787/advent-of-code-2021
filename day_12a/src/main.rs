...
fn count_paths(map: &HashMap<&str, Vec<&str>>, from: &str, to: &str) -> u64 {
    ...
    dfs_count(map, from, from, to, visited)
}

fn dfs_count(map: &HashMap<&str, Vec<&str>>, current: &str, from: &str, to: &str, visited: VecDeque<&str>) -> u64 {
    if current == to {
        return 1
    }
    ...
    for &neighbor in map.get(current).unwrap() {
        if neighbor.to_lowercase() != neighbor || !visited.contains(&neighbor) {
            ...
            sum += dfs_count(map, neighbor, from, to, neighbor_visited);
        }
    }
